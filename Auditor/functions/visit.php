...
if (isset($_POST['ubah'])) {
    ...
    $sql = "UPDATE tb_visit SET tipe_monitoring='$tipe_monitoring',tgl_monitoring='$tgl_monitoring', penyusunan_mutu_1='$penyusunan_mutu_1', penyusunan_mutu_2='$penyusunan_mutu_2', pemeriksaan_1='$pemeriksaan_1', pemeriksaan_2='$pemeriksaan_2', perubahan_kegiatan='$perubahan_kegiatan', asuransi_1='$asuransi_1', asuransi_2='$asuransi_2', pengiriman='$pengiriman', uji_coba='$uji_coba', serah_terima='$serah_terima', denda='$denda', perpanjangan='$perpanjangan', laporan='$laporan', catatan='$catatan', kriteria='$kriteria', akar_penyebab='$akar_penyebab', akibat='$akibat', rekomendasi='$rekomendasi', tanggapan_auditee='$tanggapan_auditee', rencana_perbaikan='$rencana_perbaikan' WHERE id_visit = $id_visit";
    if (mysqli_query($conn, $sql)) {
        // $success    =   "New record created successfully !";
        echo "<script>alert('Ubah Data Visit Berhasil')</script>";
        header("refresh: 0; url=timeline.php?id=" . $id_rka . "");
    } else {
        echo "Error: " . $sql . " " . mysqli_error($conn);
        // var_export(mysqli_error($conn));
        echo "<script>alert('Ubah Data Visit Gagal')</script>";
        // header("refresh: 0; url=timeline.php?id=" . $id_rka . "");
    }
    // //mysqli_close($conn);
}
