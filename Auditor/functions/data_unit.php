...
if (isset($_POST['edit'])) {
    ...
    $sql = "UPDATE tb_unit SET id_user=$ketua, nama_unit='$nama' WHERE id_unit=$id_unit";
    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('Edit unit berhasil diedit !')</script>";
        header("refresh: 0; url=data_unit.php");
    } else {
        echo "<script>alert('Unit gagal edit !')</script>";
        header("refresh: 0; url=data_unit.php");
...
if (isset($_POST['tambah'])) {
    ...
    // $q_ketua = mysqli_fetch_array(mysqli_query($conn, "SELECT id_user FROM tb_user WHERE nama = '$ketua'"));
    // $id_ketua = $q_ketua['id_user'];
    $sql = "INSERT INTO tb_unit VALUES(NULL, '$ketua', '$nama')";
    if (mysqli_query($conn, $sql)) {
        echo "<script>alert('Unit baru berhasil ditambahkan !')</script>";
        header("refresh: 0; url=data_unit.php");
    } else {
        echo "<script>alert('Unit baru gagal ditambahkan !')</script>";
        header("refresh: 0; url=data_unit.php");
        // echo "Error: " . $sql . " " . mysqli_error($conn);
    }
    //mysqli_close($conn);
